    ...
    <div class="space-y-6">
      ...
      <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p class="text-zinc-400 mt-1">Manage the plans shown on the public landing page.</p>
        </div>
      ...
      <div class="bg-black-card border border-black-border rounded-2xl overflow-hidden">
          ...
          <table class="w-full text-left border-collapse min-w-[820px]">
            <thead>
              <tr class="text-zinc-500 text-[10px] uppercase font-bold tracking-[0.2em] border-b border-black-border bg-black/20">
                ...
                <th class="px-5 py-4">Popular</th>
                <th class="px-5 py-4">Landing</th>
            ...
            <tbody class="divide-y divide-black-border/50">
              @for (plan of filteredPlans(); track plan.id) {
                <tr class="group text-zinc-300 hover:bg-gold-400/[0.02] transition-colors">
                  <td class="px-5 py-4">
                    <div class="font-bold text-white group-hover:text-gold-400 transition-colors">{{ plan.name }}</div>
                  ...
                  <td class="px-5 py-4">
                    <span [class.text-gold-400]="plan.isPopular" [class.text-zinc-600]="!plan.isPopular">{{ plan.isPopular ? 'Yes' : 'No' }}</span>
                  </td>
                  <td class="px-5 py-4">
                    <span [class.text-green-500]="plan.landingVisible" [class.text-zinc-600]="!plan.landingVisible">{{ plan.landingVisible ? 'Visible' : 'Hidden' }}</span>
                  </td>
                  <td class="px-5 py-4">
                  ...
                  <td class="px-5 py-4">
                    <div class="flex items-center justify-end gap-1">
                      <button (click)="view(plan)" title="View" class="p-2 text-gold-400 hover:bg-gold-400/20 rounded-md transition-all">
                        <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                      </button>
                      <button (click)="openEdit(plan)" title="Edit" class="p-2 text-gold-400 hover:bg-gold-400/20 rounded-md transition-all">
                        <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                      </button>
                      <button (click)="confirmDelete(plan)" title="Delete" class="p-2 text-red-500 hover:bg-red-500/20 rounded-md transition-all">
...
export class PlansComponent {
    ...
    // Client-side filter (list is small).
    ...
    async confirmDelete(plan: Plan) {
        ...
        this.planService.delete(plan.id).subscribe({
            ...
            error: (err) => this.ui.showToast(err.error?.message || 'Failed to delete plan', 'error'),
        });
    }
}
