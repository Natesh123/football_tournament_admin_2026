...
/*
  Lightweight inline-SVG icon for the Elite Pitch landing page.
  Path data is taken verbatim from Lucide (the icon set the design was mocked in),
  so we get exact glyphs without wiring the lucide-angular module / CDN.
  Size is driven by the host element via CSS (e.g. `.btn ep-icon{width:17px}`);
  `stroke` follows `currentColor`, and `fill` can be opted into for solid glyphs (stars).
*/
...
@Component({
  ...
  template: `<span class="ep-ico" [innerHTML]="svg()"></span>`,
  styles: [`
    :host { display: inline-flex; width: 24px; height: 24px; line-height: 0; }
    .ep-ico { display: inline-flex; width: 100%; height: 100%; }
    :host ::ng-deep svg { width: 100%; height: 100%; display: block; }
  `],
})
export class EpIconComponent {
  private _name = signal('');
  private _fill = signal('none');

  @Input({ required: true }) set name(v: string) { this._name.set(v); }
  /** Set to "currentColor" for solid glyphs (e.g. filled stars). */
  @Input() set fill(v: string) { this._fill.set(v || 'none'); }
  ...
  svg = computed<SafeHtml>(() => {
    ...
    const markup =
      `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="${this._fill()}" ` +
